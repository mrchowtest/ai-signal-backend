    ...
    for rec in parsed[:8]:
        ...
        try:
            if len(history) > 1:
                ...
                pct_change = f"{((end - start) / start) * 100:.2f}%"
            else:
                pct_change = "N/A"
        except Exception as e:
            print("Error calculating pct_change:", e)
            pct_change = "N/A"
        ...
        if rec['confidence'] >= 80 and history and pct_change != "N/A" and abs((float(end) - float(start)) / float(start)) >= 0.02:
